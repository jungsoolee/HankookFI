...
Var
  ...
  // 금융상품 계좌&수신처
  zAccInputItem  : pT_FI_ACCI1;
  zAccOutputItem : pT_FI_ACCO2;

  // 금융상품 보고서 전문
  zRptInputItem  : pT_FI_RPTI1;
  zRptOutputItem : pT_FI_RPTO2;
  ...
  Try
    ...
    CASE Data.dwData of
      ...
      begin
        ...
        //-- [T_FI_ACC] 금융상품 계좌 ------------------------------------------
          if (Trim(HeaderInfo.TR_NAME) = gcMCA_TR_Z_ACC) then
          begin
            // 리턴된 Input Data값
            zAccInputItem := pT_FI_ACCI1(lparam(Data.lpData) + SizeOf(TMCAHeader));

            // 리턴된 Output Data값
            zAccOutputItem := pT_FI_ACCO2(lparam(Data.lpData) + sizeof(TMCAHeader) + sizeof(pT_FI_ACCI1));

            gvMCAResult := Trim(zAccOutputItem.NRML_PRCS_YN);
          end // if (Trim(HeaderInfo.Tr_name) = gcMCA_TR_Z_ACC) then
          else

          //-- [T_FI_RPT] 금융상품 보고서 전문 ---------------------------------
          if (Trim(HeaderInfo.TR_NAME) = gcMCA_TR_Z_RPT) then
          begin
            // 리턴된 Input Data값
            zRptInputItem := pT_FI_RPTI1(lparam(Data.lpData) + SizeOf(TMCAHeader));

            // 리턴된 Output Data값
            zRptOutputItem := pT_FI_RPTO2(lparam(Data.lpData) + sizeof(TMCAHeader) + SizeOf(pT_FI_RPTI1));

            gvMCAResult := Trim(zAccOutputItem.NRML_PRCS_YN);
          end; // if (Trim(HeaderInfo.TR_NAME) = gcMCA_TR_Z_RPT) then


          //--------------------------------------------------------------------
          //  [Output Data Log]
          //--------------------------------------------------------------------
          //-- [Log] 금융상품 계좌&수신처 Import -------------------------------
          if (Trim(HeaderInfo.TR_NAME) = gcMCA_TR_Z_ACC) then
          begin
            gf_MCALog('Input.CANO           : ' + zAccInputItem.CANO);
            gf_MCALog('Input.ACNT_PRDT_CD   : ' + zAccInputItem.ACNT_PRDT_CD);
            gf_MCALog('Input.CRE_DT         : ' + zAccInputItem.CRE_DT);
            gf_MCALog('Input.CHG_DT         : ' + zAccInputItem.CHG_DT);
            gf_MCALog('Input.ITFC_ID        : ' + zAccInputItem.ITFC_ID);
            gf_MCALog('Input.POUT_FILE_NAME : ' + zAccInputItem.POUT_FILE_NAME);

            gf_MCALog('Output.NRML_PRCS_YN  : ' + zAccOutputItem.NRML_PRCS_YN);
            gf_MCALog('Output.PRCS_MSG      : ' + zAccOutputItem.PRCS_MSG);
            gf_MCALog('---------------------------------------------------------------------');
          end else
          //-- [Log] 금융상품 보고서 전문 Import -------------------------------
          if (Trim(HeaderInfo.TR_NAME) = gcMCA_TR_Z_ACC) then
          begin
            gf_MCALog('Input.CANO           : ' + zRptInputItem.CANO);
            gf_MCALog('Input.ITFC_ID        : ' + zRptInputItem.ITFC_ID);
            gf_MCALog('Input.POUT_FILE_NAME : ' + zRptInputItem.POUT_FILE_NAME);

            gf_MCALog('Output.NRML_PRCS_YN  : ' + zRptOutputItem.NRML_PRCS_YN);
            gf_MCALog('Output.PRCS_MSG      : ' + zRptOutputItem.PRCS_MSG);
            gf_MCALog('---------------------------------------------------------------------');
          end;
          //--------------------------------------------------------------------
          //  [Output Data Log]
