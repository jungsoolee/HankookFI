...
type
  TDlgForm_ERegGroup = class(TForm_Dlg)
    DRPanel1: TDRPanel;
    DRPanel2: TDRPanel;
    DRPanel6: TDRPanel;
    ...
    DRLabel3: TDRLabel;
    DREdit_AccNM: TDREdit;
    DRLabel5: TDRLabel;
    DREdit_SubCD: TDREdit;
    DRLabel1: TDRLabel;
    DRLabel7: TDRLabel;
    DREdit_Ident: TDREdit;
    Label1: TLabel;
    Label2: TLabel;
  ...
  private
    FormCreated : boolean;
    function AccSearch:Boolean;//(sFiled, sData: string): boolean;
...
var
  DlgForm_ERegGroup: TDlgForm_ERegGroup;
...
// 폼만들기
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.FormCreate(Sender: TObject);
begin
  inherited;
  FormCreated := False;
  // 그룹명
...
// FromClose
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.FormClose(Sender: TObject;
...
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.AccSelectedCheck;
var
  List_Item : TListItem;
...
// 선택 계좌 추가
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.AddSelectData;
var
...
begin
{
  ...
  // Multi Select
   ...
   begin
      ...
      begin
         AccItem := GrpAccList[i];
         if AccItem.ClientGrpYN = 'Y' then
         begin
            //ListViewSelectData;
            //AccSelectedCheck;
            gf_ShowMessage(MessageBar, mtError, 9001, 'Client Group으로 추가할 수 없습니다.'); //Database 오류
            Exit;
         end;
      end;
   end;
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.BuildTreeViewMgrAcc: boolean;
var
...
// 그룹목록 TreeView 생성
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.BuildTreeViewMgrGrp: boolean;
var iRow : integer;
begin
  Result := False;
  with ADOQuery_Temp do
...
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.ClearAllAccList;
var
  I : Integer;
...
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.ClearGrpAccList;
var
  I : Integer;
  AccItem : pTSelectAccItem;
begin
...
// 등록계좌 List에서 삭제
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DelSelectData;
var
  AccItem      : pTSelectAccItem;
  SelAccIdx, SelAccCNT, CNT  : integer;
  sClientGrpYN : String;
begin
  // Multi Select
  sClientGrpYN := '';
  ...
  begin
    ClearGrpAccList;
    CNT := DRListView_SelectAcc.SelCount;
    SelAccCNT  := DRListView_SelectAcc.Items.Count;// - DRListView_SelectAcc.SelCount);               // 선택계좌 Count
    SelAccIdx  := 0;          // 선택계좌 Index
       {
    while (CNT > 0) do
    begin
      if DRListView_SelectAcc.Items[SelAccIdx].Selected then
      begin
         sClientGrpYN := DRListView_SelectAcc.Items[SelAccidx].SubItems.Strings[1]; //ClientGrpYN
         if sClientGrpYN = 'Y' then
         begin
            gf_ShowErrDlgMessage(Self.Tag, 9001, 'Client Group 계좌로 삭제할 수 없습니다. ', 0);
            gf_ShowMessage(MessageBar, mtError, 9001, 'Client Group 계좌로 삭제할 수 없습니다.'); //Database 오류
            ListViewSelectData;
            Exit;
         end;
         Dec(CNT);
    ...
    begin
      ...
      begin
        ...
        AccItem.ClientGrpYN:= DRListView_SelectAcc.Items[SelAccIdx].SubItems.Strings[1];  // ClientGRPYN
        AccItem.ExceptFlag := DRListView_SelectAcc.Items[SelAccIdx].Checked;
        //SelAccCNT := SelAccCNT - 1;
      end // end of if
      else begin
         if (DRListView_SelectAcc.Items[SelAccidx].SubItems.Strings[1] = 'Y') then
         begin
            New(AccItem);
            GrpAccList.Add(AccItem);
            AccItem.FmtAccNo   := DRListView_SelectAcc.Items[SelAccIdx].Caption;
...
//  그룹관리 :  전체 계좌 리스트뷰 생성
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_AllAccData(Sender: TObject;
  Item: TListItem);
var
  AccItem : pTAllAccItem;
begin
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.DupDataDelete: boolean;
var
  I, Cnt     : integer;
  AccItem    : pTSelectAccItem;
  sFmtAccNo  : String;
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.ExistInSUAGPAC: boolean;
begin
  Result := False;
  with ADOQuery_Temp do
  begin
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.ExistInTreeViewMgrAcc(pGrpName: string): Integer;
var
  I : Integer;
begin
  Result := -1;
...
//------------------------------------------------------------------------------
function  TDlgForm_ERegGroup.ExistListViewSelAcc(pFmtAccNo: string) : Integer;
var
  I : Integer;
  AccItem      : pTSelectAccItem;
begin
...
// DRTreeView_MgrGrp Index Search
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.ExistInGridMgrGrp(pGrpName: string): Integer;
var
  I : Integer;
begin
  Result := -1;
...
//  그룹관리 : 등록 계좌 리스트뷰 생성
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.ListViewSelectData;
var
  I : Integer;
  AccItem : pTSelectAccItem;
  List_Item : TListItem;
  sFmtAccNo, sAccName, sClientYN : String;
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      sClientYN := AccItem.ClientGrpYN;
      List_Item.Caption  := sFmtAccNo;
      List_Item.SubItems.Add(sAccName);
      List_Item.SubItems.Add(sClientYN);
...
// 필수입력항목 check (그룹관리)
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.MgrGrpCheckKeyEditEmpty: boolean;
begin
  Result := True;
...
//  그룹관리 : 전체 계좌번호 Query
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.QueryAllAcc: integer;
var
  AccItem      : pTAllAccItem;
begin
  ...
  begin
    ...
    Close;
    SQL.Clear;
    SQL.Add('  SELECT ACC_NO, SUB_ACC_NO = '''', ACC_NAME = ACC_NAME_KOR, PARTY_COMP '
          + '        , PARTY_NAME = (SELECT ISNULL(COMP_NAME_KOR, '''')             '
          + '                        FROM SCCOMCD_TBL                               '
          + '                        WHERE SCCOMCD_TBL.COMP_CODE = SEACBIF_TBL.PARTY_COMP) '
          + '  FROM SEACBIF_TBL '
          + '  WHERE DEPT_CODE = ''' + gvDeptCode + ''' '
          + '  UNION '
    ...
    Except
      on E : Exception do
      begin  // DataBase 오류
        gf_ShowErrDlgMessage(Self.Tag, 9001, 'ADOQuery_Acc[SEACBIF_TBL]: ' + E.Message, 0);
        gf_ShowMessage(MessageBar, mtError, 9001, 'ADOQuery_Acc[SEACBIF_TBL]'); //Database 오류
        Exit;
      end;
    End;
...
// 그룹관리 : 등록 계좌 Query
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.QuerySelectAcc: boolean;
var
  AccItem      : pTSelectAccItem;
  sGrpName     : string;
  stmpStr      : string;
...
// 제외계좌 Check 표시 반영
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.SelectAccCheck: boolean;
var
  I : Integer;
  AccItem : pTSelectAccItem;
  List_Item : TListItem;
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.SelectTreeViewMgrAcc(pGrpName: string): Integer;
var
  SelectIdx : Integer;
  PreSelected : boolean;
begin
...
// 조회
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRBitBtn6Click(Sender: TObject);
begin
  inherited;
...
// 입력
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRBitBtn4Click(Sender: TObject);
var
  I       : Integer;
  AccItem :  pTSelectAccItem;
...
// 수정
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRBitBtn3Click(Sender: TObject);
var
...
// 삭제
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRBitBtn2Click(Sender: TObject);
var
...
//  TreeView의 계좌번호 선택시
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRTreeView_MgrAccChange(Sender: TObject;
  Node: TTreeNode);
var
  iMgrGrpIdx : Integer;
  AccItem      : pTSelectAccItem;
...
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRSpeedButton_FindAccClick(Sender: TObject);
begin
  inherited;
  GrpFlag := Not GrpFlag;
...
// 제외계좌 전체 선택시
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRCheckBox_ExcepAllClick(Sender: TObject);
var
  I : Integer;
  List_Item : TListItem;
begin
...
// 선택 계좌 sorting
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_SelectAccColumnClick(
...
//  DragDrop
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_SelectAccDragDrop(Sender,
...
procedure TDlgForm_ERegGroup.DRListView_SelectAccDragOver(Sender,
...
procedure TDlgForm_ERegGroup.DRListView_SelectAccEndDrag(Sender,
...
procedure TDlgForm_ERegGroup.DRListView_SelectAccMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
// 전체 계좌 sorting
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_AllAccColumnClick(Sender: TObject;
  Column: TListColumn);
var
  iCol : integer;
begin
...
//  DragDrop
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_AllAccDragDrop(Sender,
  Source: TObject; X, Y: Integer);
begin
  inherited;
...
procedure TDlgForm_ERegGroup.DRListView_AllAccDragOver(Sender,
  Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  inherited;
...
procedure TDlgForm_ERegGroup.DRListView_AllAccEndDrag(Sender,
  Target: TObject; X, Y: Integer);
begin
  inherited;
...
procedure TDlgForm_ERegGroup.DRListView_AllAccMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
// 전체계좌에서 선택계좌로 추가
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRSpeedBtn_InsertClick(Sender: TObject);
begin
  inherited;
  AddSelectData;
end;
...
// 전체계좌에서 선택계좌로 삭제
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRSpeedBtn_DeleteClick(Sender: TObject);
begin
  inherited;
  DelSelectData;
end;
...
//  폼 활성화 될때.
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.FormActivate(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    DlgForm_ERegGroup.Caption := 'Fax Group 등록';
  end else
  begin
    sCallFlag  := gcSEND_EMAIL;
    DlgForm_ERegGroup.Caption := 'Email Group 등록';
  end;
...
// 선택된 계좌 지우기
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_SelectAccKeyDown(Sender: TObject;
...
// 선택된 계좌 추가
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_AllAccDblClick(Sender: TObject);
begin
  inherited;
  AddSelectData;
end;
...
// 선택된 계좌 더블 클릭하면 지운다
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DRListView_SelectAccDblClick(Sender: TObject);
begin
  inherited;
  DelSelectData;
end;
...
// 그룹명 enter
//------------------------------------------------------------------------------
procedure TDlgForm_ERegGroup.DREdit_GrpNameKeyPress(Sender: TObject;var Key: Char);
var
  tmpGrpName : string;
...
procedure TDlgForm_ERegGroup.InputValueClear;
begin
   ClearGrpAccList;
...
procedure TDlgForm_ERegGroup.DRStrGrid_MgrGrpSelectCell(Sender: TObject;
...
procedure TDlgForm_ERegGroup.DRStrGrid_MgrGrpDblClick(Sender: TObject);
var
  iMgrGrpIdx : Integer;
  AccItem    : pTSelectAccItem;
  stmpStr    : string;
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add(' SELECT AG.ACC_NO, AG.SUB_ACC_NO,  TMP.ACC_NAME, AG.CLIENT_GRP_YN,   ');
      if sCallFlag = gcSEND_FAX then
         SQL.Add('   AG.EXCEPT_FAX, ')
      else
         SQL.Add('   AG.EXCEPT_EMAIL, ');
    ...
    begin
      ...
      AccItem.ClientGrpYN := Trim(FieldByname('CLIENT_GRP_YN').AsString);
...
procedure TDlgForm_ERegGroup.DREdit_KeyPress(Sender: TObject; var Key: Char);
var
  AccItem      : pTSelectAccItem;
begin
  inherited;
  ...
  begin
     ...
     AccSearch;//('IDENTIFICATION', DREdit_Ident.text);
...
// 검색
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.AccSearch: boolean;//(sFiled, sData : string): boolean;
var
  sFilter, sAccNo, sSubAccNo, sSubDirectGB: string;
  iRow : integer;
  AccItem : pTAllAccItem;
begin
  Result := False;
  ...
  begin
      gf_UnformatAccNo(Trim(DREdit_AccNo.Text), sAccNo, sSubAccNo);
      ...
      sFilter := 'ACC_NO like ''' + sAccNo + '%''';
  end;

  //계좌명
  if Trim(DREdit_AccNM.Text) <> '' then
  begin
      if sFilter <> '' then
         sFilter := sFilter + ' AND ';

      sFilter := sFilter + ' ACC_NAME_KOR like ''' + Trim(DREdit_AccNM.Text) + '%''';
  end;

  //SUBCODE
  if Trim(DREdit_SubCD.Text) <> '' then
  begin
      if sFilter <> '' then
         sFilter := sFilter + ' AND ';

      sFilter := sFilter + ' INV_ACC_NO like ''' + Trim(DREdit_SubCD.Text) + '%''';
  end;

  //ID
  if Trim(DREdit_Ident.Text) <> '' then
  begin
      if sFilter <> '' then
         sFilter := sFilter + ' AND ';

      sFilter := sFilter + ' IDENTIFICATION like ''' + Trim(DREdit_Ident.Text) + '%''';
  end;
  ...
  begin
    ...
    begin
      SQL.Add(' select SESACIF_TBL.ACC_NO, SESACIF_TBL.SUB_ACC_NO, ACC_NAME = SUB_NAME_KOR, PARTY_COMP, BOOK_NAME = SESACIF_TBL.SUB_BOOK_KOR '
            + '        , PARTY_NAME = (SELECT ISNULL(COMP_NAME_KOR, '''')             '
            + '                        FROM SCCOMCD_TBL                               '
            + '                        WHERE SCCOMCD_TBL.COMP_CODE = SEACBIF_TBL.PARTY_COMP )'
            + ' from SESACIF_TBL, SEACBIF_TBL                                              '
            + ' where SESACIF_TBL.DEPT_CODE = ''' + gvDeptCode + '''                      '
            + ' and   SESACIF_TBL.ACC_NO    = ''' + sAccNo    + ''' '
            + ' and   SESACIF_TBL.SUB_ACC_NO= ''' + sSubAccNo + ''' '
            + ' and   SEACBIF_TBL.DEPT_CODE = SESACIF_TBL.DEPT_CODE '
            + ' and   SEACBIF_TBL.ACC_NO    = SESACIF_TBL.ACC_NO    '
            + ' ORDER BY SESACIF_TBL.ACC_NO, SESACIF_TBL.SUB_ACC_NO '
            );
    end
    else
    begin
      if sFilter <> '' then
      begin
         SQL.Add(' select ACC_NO, SUB_ACC_NO = '''', ACC_NAME = ACC_NAME_KOR, PARTY_COMP, BOOK_NAME = BOOK_NAME_KOR '
               + '        , PARTY_NAME = (SELECT ISNULL(COMP_NAME_KOR, '''')             '
               + '                        FROM SCCOMCD_TBL                               '
               + '                        WHERE SCCOMCD_TBL.COMP_CODE = SEACBIF_TBL.PARTY_COMP) '
               + ' from SEACBIF_TBL                                              '
               ...
               + ' and   (' + sFilter + ')'
               + ' union                                                         '
               + ' select SESACIF_TBL.ACC_NO, SESACIF_TBL.SUB_ACC_NO, ACC_NAME = SUB_NAME_KOR, PARTY_COMP, BOOK_NAME = SESACIF_TBL.SUB_BOOK_KOR '
               + '        , PARTY_NAME = (SELECT ISNULL(COMP_NAME_KOR, '''')             '
               + '                        FROM SCCOMCD_TBL                               '
               + '                        WHERE SCCOMCD_TBL.COMP_CODE = SEACBIF_TBL.PARTY_COMP )'
               + ' from SESACIF_TBL, SEACBIF_TBL                                              '
               + ' where SESACIF_TBL.DEPT_CODE = ''' + gvDeptCode + '''                      '
               + ' and   (SEACBIF_TBL.' + sFilter + ')'
               + ' and   SEACBIF_TBL.DEPT_CODE = SESACIF_TBL.DEPT_CODE '
               + ' and   SEACBIF_TBL.ACC_NO = SESACIF_TBL.ACC_NO '
               + ' ORDER BY ACC_NO, SUB_ACC_NO                                   '
               );
...
procedure TDlgForm_ERegGroup.EditWndProc(var Message: TMessage);
begin
   case Message.Msg of
...
//------------------------------------------------------------------------------
function TDlgForm_ERegGroup.SelectGridMgrGrp(pGrpName: string): Integer;
var
  SelectIdx : Integer;
  PreSelected : boolean;
begin
