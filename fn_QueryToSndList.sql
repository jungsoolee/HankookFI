...
SELECT  SEND_TYPE = 'F', M.EMP_NO, U.USER_NAME, M.JOB_TIME,
	     ...
	     RCV_COMP_KOR = '', MAIL_ADDR = '',
	     R.REPORT_CODE, R.VIEW_FILENAME, SUBJECT_DATA = '', MAIL_BODY_DATA = ''
...
UNION
-- E-MAIL 전송 대상 쿼리
SELECT  SEND_TYPE = 'E', M.EMP_NO, U.USER_NAME, M.JOB_TIME,
		 ACC_NO = (CASE WHEN M.PRD_NO = '' THEN M.ACC_NO
	  				    ELSE (CASE WHEN M.BLC_NO = '' THEN M.ACC_NO + '-' + M.PRD_NO
								  ELSE M.ACC_NO + '-' + M.PRD_NO + '-' + M.BLC_NO
							  END)
				   END), A.ACC_NAME_KOR,
		 RCV_NAME_KOR = '', MEDIA_NO = '',		   
	     E.RCV_COMP_KOR, E.MAIL_ADDR, 
	     R.REPORT_CODE, R.VIEW_FILENAME, R.SUBJECT_DATA, CONVERT(VARCHAR(8000), R.MAIL_BODY_DATA)
FROM SZMAIN_INS M JOIN SZACBIF_INS A
ON M.DEPT_CODE = A.DEPT_CODE
AND M.ACC_NO = A.ACC_NO
JOIN SZMELDE_INS E
...
UNION
-- 전송대상 미전송 리스트
SELECT  SEND_TYPE = 'N', M.EMP_NO, U.USER_NAME, M.JOB_TIME,
		 ACC_NO = (CASE WHEN M.PRD_NO = '' THEN M.ACC_NO
	  				    ELSE (CASE WHEN M.BLC_NO = '' THEN M.ACC_NO + '-' + M.PRD_NO
								  ELSE M.ACC_NO + '-' + M.PRD_NO + '-' + M.BLC_NO
							  END)
